...
toml_parse_doc :: proc(content: string) -> (TomlDoc, bool) {
    ...
    for idx < len(tokens) {
        ...
        case .IDENTIFIER:
            ...
            // Set the value with table prefix
            if len(current_table) > 0 {
                final_key := fmt.aprintf("%s.%s", current_table, full_key)
    ...
    if constants_val != nil && constants_val.type == .ARRAY {
        for const_table in constants_val.arr_val {
            ...
            if len(constant.name) > 0 {
                append(&config.constants, constant)
            }
        }
    }
        ...
        for profile_name, profile_table in profiles_val.table_val {
            ...
            if constants_val, ok := profile_table.table_val["constants"]; ok && constants_val.type == .ARRAY {
                for const_table in constants_val.arr_val {
                    ...
                    if len(constant.name) > 0 {
                        append(&profile.constants, constant)
                    }
                }
            }
...
toml_parse :: proc(content: string) -> (TomlConfig, bool) {
    doc, ok := toml_parse_doc(content)
    if !ok {
        return {}, false
    }
    defer destroy_toml_doc(&doc)
...
toml_merge_profiles :: proc(base: TomlConfig, profile_name: string) -> TomlConfig {
    ...
    for alias in profile.aliases {
        append(&merged.aliases, alias)
    }
    
    // Append profile constants
    for constant in profile.constants {
        append(&merged.constants, constant)
    }
    
    // Append profile plugins
    for plugin in profile.plugins {
        append(&merged.plugins, plugin)
    }
