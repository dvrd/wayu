...
test_matches_source_all_passes_everything :: proc(t: ^testing.T) {
	testing.expect(t, wayu.matches_source("● /some/path", .ALL), "ALL must pass active item")
	testing.expect(t, wayu.matches_source("○ /external",  .ALL), "ALL must pass external item")
	testing.expect(t, wayu.matches_source("─── External (3) ───", .ALL), "ALL must pass separator")
	testing.expect(t, wayu.matches_source("", .ALL), "ALL must pass empty string")
}

@(test)
test_matches_source_wayu_active_only_dot :: proc(t: ^testing.T) {
	testing.expect(t, wayu.matches_source("● /path", .WAYU_ACTIVE), "● must match WAYU_ACTIVE")
	testing.expect(t, !wayu.matches_source("⚠ /path", .WAYU_ACTIVE), "⚠ must NOT match WAYU_ACTIVE")
	testing.expect(t, !wayu.matches_source("○ /path", .WAYU_ACTIVE), "○ must NOT match WAYU_ACTIVE")
	testing.expect(t, !wayu.matches_source("─── sep ───", .WAYU_ACTIVE), "separator must NOT match WAYU_ACTIVE")
	testing.expect(t, !wayu.matches_source("", .WAYU_ACTIVE), "empty must NOT match WAYU_ACTIVE")
}

@(test)
test_matches_source_external_only_circle :: proc(t: ^testing.T) {
	testing.expect(t, wayu.matches_source("○ /usr/bin", .EXTERNAL), "○ must match EXTERNAL")
	testing.expect(t, !wayu.matches_source("● /path", .EXTERNAL), "● must NOT match EXTERNAL")
	testing.expect(t, !wayu.matches_source("⚠ /path", .EXTERNAL), "⚠ must NOT match EXTERNAL")
}

@(test)
test_matches_source_inactive_only_warn :: proc(t: ^testing.T) {
	testing.expect(t, wayu.matches_source("⚠ /missing", .WAYU_INACTIVE), "⚠ must match WAYU_INACTIVE")
	testing.expect(t, !wayu.matches_source("● /path", .WAYU_INACTIVE), "● must NOT match WAYU_INACTIVE")
	testing.expect(t, !wayu.matches_source("○ /ext", .WAYU_INACTIVE), "○ must NOT match WAYU_INACTIVE")
}
...
test_has_any_filter_reflects_source_alone :: proc(t: ^testing.T) {
	...
	testing.expect(t, !wayu.has_any_filter(&state), "Fresh state must report no filter")

	state.source_filter = .EXTERNAL
	testing.expect(t, wayu.has_any_filter(&state), "Source filter alone must count as active filter")
}
...
test_parse_source_token_extracts_value_and_strips :: proc(t: ^testing.T) {
	rem, src, ok := wayu.parse_source_token("source:external")
	...
	rem2, src2, ok2 := wayu.parse_source_token("bin source:wayu")
	...
	rem3, src3, ok3 := wayu.parse_source_token("source:inactive /usr")
	...
	rem4, _, ok4 := wayu.parse_source_token("source:bogus rest")
