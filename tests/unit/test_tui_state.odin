...
test_move_selection_large_delta :: proc(t: ^testing.T) {
	...
	wayu.tui_state_move_selection(&state, 10, item_count)
	// (1 + 10) = 11, 11 % 7 = 4
...
test_move_selection_zero_items :: proc(t: ^testing.T) {
	...
	wayu.tui_state_move_selection(&state, 1, 0)
...
test_scroll_update_up :: proc(t: ^testing.T) {
	...
	wayu.tui_state_move_selection(&state, 0, item_count)  // Trigger scroll update
...
test_needs_refresh_flag :: proc(t: ^testing.T) {
	...
	wayu.tui_state_move_selection(&state, 1, 7)
...
test_all_view_transitions :: proc(t: ^testing.T) {
	...
	for view in views {
		wayu.tui_state_goto_view(&state, view)
