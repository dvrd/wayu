...
generate_eval_output_optimized :: proc() {
	...
	// 4. Pre-computed tools (Starship, Zoxide, Atuin) - FAST, no subprocess
	append_precomputed_tools(&builder)
	
	// 5. Load plugins inline (fast, no external file)
	...
	// 6. Source external configs (completions, functions, lazy tools)
...
// Append pre-computed tool initializations from cache
// Reads cached eval output from ~/.cache/wayu/*.zsh
// Cache is generated by tools.zsh on first load
append_precomputed_tools :: proc(builder: ^strings.Builder) {
	home := os.get_env("HOME", context.temp_allocator)
	cache_dir := fmt.aprintf("%s/.cache/wayu", home)
	defer delete(cache_dir)
	
	tools := []string{"starship", "zoxide", "atuin"}
	
	fmt.sbprintln(builder, "# Pre-computed tool initializations (cached)")
	
	for tool in tools {
		cache_file := fmt.aprintf("%s/%s.zsh", cache_dir, tool)
		defer delete(cache_file)
		
		if os.exists(cache_file) {
			content, ok := safe_read_file(cache_file)
			if ok && len(content) > 0 {
				fmt.sbprintfln(builder, "# %s", tool)
