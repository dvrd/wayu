...
// Note: DirenvConfig defined in interfaces.odin
    ...
    if len(config.constants) > 0 {
        ...
        for constant in config.constants {
            if constant.export {
                ...
                if constant.secret {
                    fmt.sbprintf(&sb, "# export %s=*** (secret value hidden)\n", constant.name)
                } else {
                    // Escape value for shell
                    escaped := escape_shell_value(constant.value)
                    fmt.sbprintf(&sb, "export %s=%s\n", constant.name, escaped)
                    delete(escaped)
                }
            }
        }
...
load_config_or_default :: proc() -> TomlConfig {
    ...
    if os.exists(config_path) {
        content_bytes, err := os.read_entire_file_from_path(config_path, context.allocator)
        if err == nil {
            defer delete(content_bytes)
            content := string(content_bytes)
            config, ok := toml_parse(content)
            if ok {
                return config
            }
        }
